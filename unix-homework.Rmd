...
```{bash}
cd  ~/Bioinformatics2020_CSU/unix-play-with-genomic-data-cbossu
pwd
```
...
```{bash}
cp fastq/*R1*gz lonely-fastqs/R1
cp fastq/*R2*gz lonely-fastqs/R2
...
```{bash}
cp fastq/DPCh_plate1_[A-B]*R1*gz fq_AB/
```
...
```{bash}
cp fastq/DPCh_plate1_[^A-B]*R1*gz fq_notAB
ls -l fq_AB fq_notAB
```
...
```{bash}
mkdir -p "bad directory name with spaces"
...
```
Need to confirm you are only deleting the directories in your path, and not other files that also have "dir" in their name 
```

# If you had just typed `rm -r bad directory name with spaces` what would Unix have tried to remove?

```
Unix would have tried to remove a directory named bad, that does not exist
```
...
```
They are the same individual, just different regions of the genome (Line1=1101:1824:48192 and Line2=1101:2869:22080), for read1 and read2. I'm not sure if it's chr 1 or scaffold 1101. 
```
...
```{bash}
gzcat fastq/*R1*gz > R1_all_via_gzcat.fq
```
...
```{bash}
cat fastq/*R1*gz > R1_all_via_cat.fq.gz
```
...
```{bash}
gzip -c R1_all_via_gzcat.fq > R1_all_via_gzcat.fq.gz
...
```
~4.76, so it saves approximately 5x the space
```
